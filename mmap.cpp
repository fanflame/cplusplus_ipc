//
// Created by fanqiang on 2020/3/15.
// 内存映射
//
//
#include <sys/mman.h>
...
struct myinfo *get_mmap(size_t length) {
    int fd = open("./mmaptest", O_CREAT | O_RDWR);
    ftruncate(fd,length);
    struct myinfo *mm = (struct myinfo *) mmap(nullptr, length, PROT_WRITE | PROT_READ, MAP_SHARED, fd, 0);
    //第二个参数，内存映射的大小
